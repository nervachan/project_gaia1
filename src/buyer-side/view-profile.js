...
document.addEventListener('DOMContentLoaded', function() {
  ...
  async function openProfileModal(username) {
    ...
    try {
      // Query Firestore for the user document matching the 'username' field
      const q = query(collection(db, 'user-buyer'), where('username', '==', username));  // Querying based on the 'username' field
  ...
  onAuthStateChanged(auth, (user) => {
    if (user) {
      // User is signed in
      const loggedInUsername = user.displayName || 'defaultUsername';  // You may need to use `displayName` or `email` as the logged-in user's username

      // Simulate opening the modal when the profile icon is clicked
      document.getElementById('profile-icon').addEventListener('click', function(event) {
        event.preventDefault();  // Prevent the default link behavior
        openProfileModal(loggedInUsername);  // Pass the logged-in user's username to the function
      });
    } else {
